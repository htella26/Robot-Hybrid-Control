...
function h = create_new_robot(robot, opt)
    ...
    if ~ishold
         if isfield(opt, 'dims') && numel(opt.dims) == 6
             % axis(opt.dims);
             axis auto
         else
    ...
    set(gca, 'SortMethod', 'childorder');  % Use the modern property for rendering order
	grid on
	...
	if opt.name && isfield(robot, 'name') && ischar(robot.name)
		b = transl(robot.base);
		text(b(1), b(2)-opt.mag, [' ' robot.name]);
	else
		warning('Robot name is invalid or not set.');
	end
